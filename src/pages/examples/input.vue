<template>
  <div class="page">
    ...
    <wv-group title="带验证">
      ...
      <wv-input label="失焦时验证" placeholder="请输入内容" v-model="valueOnBlur" required :validate-mode="{onFocus: false, onBlur: true, onChange: false}"/>
    </wv-group>

    <wv-group title="只读">
      <wv-input label="只读内容" placeholder="请输入内容" :readonly="true" v-model="valueReadonly"/>
    </wv-group>

    <wv-group title="自定义标签宽度">
      <wv-input label="这是一个比较长的标签" :label-width="190" placeholder="请输入内容" v-model="valueText"/>
    </wv-group>
